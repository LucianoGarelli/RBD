...
## importo hdf5
h5f = h5py.File('./Resultados_Case_4/Data.hdf5','r')
# parametros hdf5
...
t = h5f['/Time'][:]
h5f.close()
header = ['t', 'x', 'y', 'z', 'p','q','r','vxb','vyb','vzb','mag(v)','phi','theta','psi','vx','vy','vz']
data = np.column_stack([np.transpose(t), x,w,vb,np.linalg.norm(vned,axis=1),euler_ang*180/math.pi,vned])
...
def plot_force():
    fig_size = (12,4)
    xy = np.array([[0],[0],[0],[0]])
    f = open("./Resultados_Case_4/Forces_proc.txt", "r")
    line1 = f.readline()
    for line in f:
        frags = line.split(',')
...
def plot_moments():
    fig_size = (12, 4)
    xy = np.array([[0], [0], [0], [0]])
    f = open("./Resultados_Case_4/Moments_proc.txt", "r")
    line1 = f.readline()
    for line in f:
        frags = line.split(',')
