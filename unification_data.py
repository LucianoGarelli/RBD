...
# vel "x" en NED
data = []
data = np.loadtxt('UE_BFP.csv', delimiter=',', skiprows=1)
t_vxned = data[:,0]
vxned = data[:,1]
# interpolated data
...
# vel "y" en NED
data = []
data = np.loadtxt('VE_BFP.csv', delimiter=',', skiprows=1)
t_vyned = data[:,0]
vyned = data[:,1]
# interpolated data
...
# vel "z" en NED
data = []
data = np.loadtxt('we_BFP.csv', delimiter=',', skiprows=1)
t_vzned = data[:,0]
vzned = data[:,1]
# interpolated data
...
data = []
data = np.loadtxt('XE_QFW.csv', delimiter=',', skiprows=1)
t_xe = data[:,0]
xe = data[:,1]
# interpolated data
...
# y - recorrido  lateral
data = []
data = np.loadtxt('YE_QFW.csv', delimiter=',', skiprows=1)
t_ye = data[:,0]
ye = data[:,1]
# interpolated data
...
# Directory Set up (Resu): Data.hdf5, Forces_prop_unificated.txt, Moment_prop_unificated.txt
MYDIR = ("Resultados")
...
ff = open("./Resultados/Forces_proc.txt", "w")
ff.write("# Time,     alpha,     beta,     V_inf,    V_inf, V_inf, V_inf,  p, "
         "      q,         r,         gx,         gy,       gz,      FX,         FY,         FZ       \n")
ff.close()

ff = open('./Resultados/Forces_proc.txt', 'ab')
...
fm = open("./Resultados/Moments_proc.txt", "w")
fm.write("# Time,     alpha,      beta,     p,      q,      r,    MX,     MY,     MZ \n")
fm.close()

fm = open('./Resultados/Moments_proc.txt', 'ab')
